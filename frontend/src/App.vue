...
const players = ref<Record<string, { score: number }>>({});
...
function createMultiplayerGame(diff: 4 | 5 | 6) {
  loading.value = true;
  error.value = null;
  isMultiplayer.value = true;
  socket.emit('createRoom', { difficulty: diff });
}
...
function joinMultiplayerGame() {
  if (roomIdToJoin.value.trim()) {
    ...
    error.value = null;
    isMultiplayer.value = true;
    socket.emit('joinRoom', { roomId: roomIdToJoin.value.trim() });
  }
}
...
onMounted(() => {
  ...
  socket.on('gameUpdate', (data) => {
    ...
    players.value = data.players;
    multiplayerFoundWords.value = data.foundWords;
    loading.value = false;
  });
  ...
  socket.on('error', (data) => {
    ...
    loading.value = false;
    setTimeout(() => {
      if (gameStarted.value && isMultiplayer.value && !roomId.value) {
        goBackToMenu();
      } else {
        error.value = null;
      }
  ...
  <main :class="currentTheme">
    <header>
      ...
      <div class="header-actions">
        <button v-if="gameStarted && isMultiplayer" @click="toggleGameInfoPopup" class="header-btn">
          👥
        </button>
    ...
    <div v-if="!gameStarted" class="level-selector">
      <div class="difficulty-selector">
        ...
        <div class="difficulty-buttons">
          <button v-for="diff in [4, 5, 6]" :key="diff" @click="createNewPuzzle(diff as 4 | 5 | 6)" class="difficulty-btn">
            {{ diff }} Harf
          </button>
        </div>
      </div>
      <div class="multiplayer-menu">
        ...
        <div class="join-room-container">
          <input type="text" v-model="roomIdToJoin" placeholder="Oda Kodu" class="room-input" @keyup.enter="joinMultiplayerGame"/>
          <button @click="joinMultiplayerGame" class="join-btn">Odaya Katıl</button>
        </div>
        <p>veya yeni bir oda kur:</p>
        <div class="difficulty-buttons">
          <button v-for="diff in [4, 5, 6]" :key="`mp-${diff}`" @click="createMultiplayerGame(diff as 4 | 5 | 6)" class="difficulty-btn-mp">
            {{ diff }} Harfli Oda
          </button>
        </div>
      </div>
              ...
              <div v-if="isMultiplayer" class="popup-section">
                ...
                <ul class="popup-players-list">
                  <li v-for="(player, id) in players" :key="id">
                    <span>Oyuncu {{ id.substring(0, 4) }}...</span>
...
header { display: flex; align-items: center; justify-content: center; padding: 1rem; flex-shrink: 0; position: relative; width: 100%; box-sizing: border-box; }
...
/* DEĞİŞİKLİK: Yeni header buton sarmalayıcısı */
.header-actions {
  position: absolute;
  right: 1rem;
  top: 50%;
...
.theme-color-dot { width: 20px; height: 20px; border-radius: 50%; border: 2px solid var(--button-border); }
h1 { font-size: clamp(1.2rem, 5vw, 2rem); margin: 0; text-align: center; flex-grow: 1; padding: 0 150px; /* Butonlar için daha fazla boşluk */ box-sizing: border-box; }
...
.difficulty-btn:hover { background: var(--button-hover-bg); transform: translateY(-2px); }
...
.difficulty-btn-mp:hover { background: var(--button-hover-bg); }
.success-buttons { display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap; margin-top: 1rem; }
.next-btn, .restart-btn { padding: 0.8rem 1.5rem; font-size: 1rem; cursor: pointer; border: none; border-radius: 8px; transition: all 0.3s ease; font-weight: bold; }
...
.restart-btn:hover { background-color: var(--button-hover-bg); }
.loading, .error { text-align: center; padding: 2rem; font-size: 1.2rem; }
.error { color: var(--error-color); }
.game-container { display: flex; flex-direction: column; flex: 1; gap: 1rem; padding: 0.5rem; min-height: 0; overflow: hidden; }
...
.game-info-popup { background-color: var(--background-color); padding: 2rem; border-radius: 12px; border: 1px solid var(--button-border); width: 90%; max-width: 400px; position: relative; display: flex; flex-direction: column; gap: 1.5rem; }
...
.popup-section p { margin: 0 0 1rem; font-size: 0.9rem; opacity: 0.8; }
...
@media (max-width: 768px) {
    .level-selector { gap: 1rem; padding-top: 0; }
}
</style>
